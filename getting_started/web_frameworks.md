...
[Peko](https://peko.deno.dev/) is a server framework with a functional routing API and a library of middleware and utility classes for logic such as static-file hosting or cryptography. 

The framework is flexible, entensible and lightning fast due to its focus on utilising native Deno & JS features with no dependencies outside of the Deno std library.
